 ...
 * 文件大小对比测试
 * 
 * <p>对比POI、EasyExcel生成的XLSX与jxlsb生成的XLSB文件大小。
 * 
 * @author AI架构师
 * @since 1.0.0
 */
...
public class FileSizeComparisonTest {
    ...
    private void generatePoiFile(Path file, int rows, int cols, 
                                 List<TestData> testDatas) throws IOException {
        try (SXSSFWorkbook workbook = new SXSSFWorkbook(1000)) {
            ...
            for (int row = 0; row < rows; row++) {
                ...
                for (int col = 0; col < cols; col++) {
                    ...
                    switch (col % 4) {
                        case 0:
                            cell.setCellValue(data.product + "-" + col);
                            break;
                        case 1:
                            cell.setCellValue(data.price + col);
                            break;
                        case 2:
                            cell.setCellValue(new Date(data.timestamp));
                            break;
                        case 3:
                            cell.setCellValue(data.active);
                            break;
                    }
                }
            }
    ...
    private void generateEasyExcelFile(Path file, int rows, int cols,
                                       List<TestData> testDatas) throws IOException {
        ...
        for (int row = 0; row < rows; row++) {
            ...
            for (int col = 0; col < cols; col++) {
                switch (col % 4) {
                    case 0:
                        rowData.add(data.product + "-" + col);
                        break;
                    case 1:
                        rowData.add(data.price + col);
                        break;
                    case 2:
                        rowData.add(new Date(data.timestamp));
                        break;
                    case 3:
                        rowData.add(data.active);
                        break;
                    default:
                        rowData.add(null);
                }
        ...
        try (XlsbWriter writer = XlsbWriter.builder()
            ...
            writer.writeBatch("Sheet1",
                (row, col) -> {
                    switch (col % 4) {
                        case 0:
                            return CellData.text("Product-" + row + "-" + col);
                        case 1:
                            return CellData.number(row * 100.50 + col);
                        case 2:
